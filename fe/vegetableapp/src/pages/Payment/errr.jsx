...
const PaymentPage = ({ count = 1 }) => {
  ...
  const shippingFee = shippingMethod === 'standard' ? 10000 : 20000;

  const handleAddOrder = () => {
    console.log('User:', user);
    console.log('Order Select:', order?.orderSelect);
  ...
  const handlePlaceOrder = async () => {
    if (!shippingMethod || !paymentMethod) {
      ...
      try {
        ...
        const response = await OrderService.createOrder(user?.access_token, orderData);

        // Kiểm tra nếu đặt hàng thành công
        if (response.status === 'OK') {
          message.success('Đặt hàng thành công');
          // Thực hiện bất kỳ hành động nào cần thiết sau khi đặt hàng thành công,
          // Ví dụ: navigate đến trang thành công
          navigate('/success');
        } else {
        ...
        message.error('Đặt hàng ko đc. Vui lòng thử lại sau.');
      }
    }
  };
